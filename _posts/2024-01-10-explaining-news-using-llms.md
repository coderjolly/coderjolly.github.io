...
Firstly, we use OpenAI's GPT3.5 to summarise the provided text because of its training model optimised by human feedback. Summarisation has the ability to not only shorten the length, but also improve the semantic details that fail to be noticed in lines of text. Then, we again use OpenAI's GPT3.5 for keyword extraction with a different custom in-context prompt based on their semantics so instead of the basic document grouping mode, we choose the topic modelling mode implemented with the K-means algorithm to find the embeddings. This approach helps in clustering with some descriptive labels of each cluster. And lastly, we use GPT3.5 to extract a theme for those clusters. The industry explorations reveal that the theme extracted by OpenAI were more meaningful and closer to reflect the gist of text provided. And finally, for a better understanding, images are fetched based on each keyword/keyphrase using Google Image Search.
